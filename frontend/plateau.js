...
const CONFIG = {
  ...
  plateau: {
    ...
    buildings:
      "https://assets.cms.plateau.reearth.io/assets/3d/tiles/23ku/tileset.json",
  ...
  cameraPresets: {
    ...
    shibuya: { center: [139.7016, 35.658], zoom: 16, pitch: 65, bearing: 30 },
    shinjuku: {
      center: [139.7005, 35.6896],
      zoom: 16,
      pitch: 60,
      bearing: -45,
    },
    "birds-eye": { center: [139.75, 35.68], zoom: 12, pitch: 0, bearing: 0 },
  },
...
map.on("load", async () => {
  ...
  try {
    map.addSource("terrain-dem", {
      ...
      tiles: [
        "https://assets.cms.plateau.reearth.io/assets/terrain/dem/{z}/{x}/{y}.png",
      ],
      tileSize: 256,
      maxzoom: 15,
    });
    ...
    console.warn(
      "⚠️ Terrain source unavailable, using flat terrain:",
      e.message
    );
  }
  ...
  try {
    ...
    map.addLayer({
      ...
      paint: {
        "fill-extrusion-color": [
          ...
          ["get", "measuredHeight"],
          0,
          "#1a237e", // Low buildings — deep blue
          30,
          "#283593", // Medium
          60,
          "#3949ab", // Tall
          ...
          "#5c6bc0", // Skyscrapers
          300,
          "#7986cb", // Very tall (Toranomon, etc.)
        ],
    ...
    console.warn(
      "⚠️ PLATEAU vector tiles failed, trying alternative method...",
      e.message
    );
...
function generateMockDrivers(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const lng = 139.7 + Math.random() * 0.12;
...
function generateMockOrders(count) {
  ...
  for (let i = 0; i < count; i++) {
    orders.push({
      id: `order-${i}`,
      coordinates: [139.7 + Math.random() * 0.12, 35.65 + Math.random() * 0.08],
    ...
    if (idx < orders.length) {
      ...
      routes.push({
        ...
        coordinates: [
          ...
          [
            (driver.coordinates[0] + order.coordinates[0]) / 2 +
              (Math.random() - 0.5) * 0.01,
            (driver.coordinates[1] + order.coordinates[1]) / 2 +
              (Math.random() - 0.5) * 0.01,
          ],
          order.coordinates,
        ],
...
document.getElementById("speed-slider").addEventListener("input", (e) => {
  ...
  document.getElementById(
    "speed-val"
  ).textContent = `${CONFIG.simulation.speed}x`;
});
...
function showDriverPanel(props) {
  ...
  info.innerHTML = `
        <div class="stat"><span class="label">Name</span><span class="value">${
          props.name
        }</span></div>
        <div class="stat"><span class="label">Status</span><span class="value">${
          props.status
        }</span></div>
        <div class="stat"><span class="label">Speed</span><span class="value">${props.speed.toFixed(
          1
        )} km/h</span></div>
        <div class="stat"><span class="label">Fuel</span><span class="value">${props.fuel.toFixed(
          0
        )}%</span></div>
        <div class="stat"><span class="label">Earnings</span><span class="value">¥${props.earnings.toLocaleString()}</span></div>
        <div class="stat"><span class="label">Deliveries</span><span class="value">${
          props.deliveries
        }</span></div>
    `;
