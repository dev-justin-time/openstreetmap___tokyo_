...
export function setInfoPanelVisible(visible) {
  try {
    ...
    if (hudEl)
      hudEl.style.display = appConfig.GUI_INFO_PANEL_VISIBLE ? "" : "none";
    const toggle = document.getElementById("hud-toggle");
    if (toggle)
      toggle.setAttribute(
        "aria-pressed",
        appConfig.GUI_INFO_PANEL_VISIBLE ? "false" : "true"
      );
  } catch (e) {
    /* ignore */
  }
...
export function updateHUD() {
  if (appConfig.GUI_IMPERIAL) {
    ...
    hudFuel.textContent = `${appConfig.UI_LABELS.fuel}: ${(
      fuelLiters * 0.264172
    ).toFixed(1)} ${appConfig.UI_LABELS.gallons}`;
  } else {
    hudSpeed.textContent = `${appConfig.UI_LABELS.speed}: ${currentSpeedKmph} ${appConfig.UI_LABELS.kmph}`;
    hudFuel.textContent = `${appConfig.UI_LABELS.fuel}: ${fuelLiters.toFixed(
      1
    )} ${appConfig.UI_LABELS.liters}`;
  }
  ...
  if (!hudMoney) {
    ...
    followBtn.textContent = `${appConfig.UI_LABELS.follow}: ${
      followCar ? appConfig.UI_LABELS.on : appConfig.UI_LABELS.off
    }`;
    followBtn.addEventListener("click", () => {
      ...
      followBtn.textContent = `${appConfig.UI_LABELS.follow}: ${
        current ? appConfig.UI_LABELS.on : appConfig.UI_LABELS.off
      }`;
      if (current) {
    ...
    if (hudToggle) {
      ...
      hudToggle.textContent = appConfig.GUI_INFO_PANEL_VISIBLE
        ? appConfig.UI_LABELS.hide
        : appConfig.UI_LABELS.show;
      ...
      hudToggle.addEventListener(
        ...
        () => {
          ...
          const hudEl = document.getElementById("hud");
          if (hudEl)
            hudEl.style.display = appConfig.GUI_INFO_PANEL_VISIBLE
              ? ""
              : "none";
        },
        { passive: true }
  ...
  hudMoney.textContent = `${appConfig.UI_LABELS.money}: ${
    appConfig.currency
  } ${money.toFixed(2)}`;
  ...
  hudRoute.textContent = `${appConfig.UI_LABELS.route}: ${
    CURRENT_ROUTE_NAME || "—"
  }`;
  ...
  hudRoad.textContent = `${appConfig.UI_LABELS.type}: ${
    CURRENT_ROAD_TYPE || "—"
  }`;

  // Country display
  if (hudCountry) {
    // Pre-existing, just update content and visibility
  ...
  if (appConfig.GUI_SHOW_REMAINING_DISTANCE) {
    ...
    try {
      if (routeCoords && routeCoords.length >= 2) {
        remainingMeters = distanceToIndexFraction(
          routeIndexFloat || 0,
          Math.max(0, routeCoords.length - 1),
          routeCoords
        );
      }
    } catch (e) {
    ...
    hudDist.textContent = `${
      appConfig.UI_LABELS.remaining
    }: ${formatDistanceForDisplay(remainingMeters, appConfig.GUI_IMPERIAL)}`;
  } else if (hudDist) {
    hudDist.remove();
    hudDist = null;
  }
  ...
  if (appConfig.GUI_SHOW_ETA) {
    ...
    try {
      ...
      if (
        vehicleSpeedMps &&
        vehicleSpeedMps >= appConfig.GUI_ETA_MIN_SPEED_MPS
      ) {
        ...
        if (routeOriginalCoords && routeOriginalCoords.length >= 2) {
          ...
          for (let i = 0; i < routeOriginalCoords.length - 1; i++) {
            totalMeters += map.distance(
              routeOriginalCoords[i],
              routeOriginalCoords[i + 1]
            );
          }
          if (totalMeters > 0) {
            const frac = Math.min(
              1,
              Math.max(0, remainingMeters / totalMeters)
            );
      ...
      if (etaSec != null && isFinite(etaSec)) {
        ...
        let mins;
        if (appConfig.GUI_ETA_MINUTES_ROUNDING === "ceil")
          mins = Math.ceil(rawMins);
        else if (appConfig.GUI_ETA_MINUTES_ROUNDING === "floor")
          mins = Math.floor(rawMins);
        ...
        if (mins >= 60) {
          ...
          etaText =
            rem === 0
              ? `${hrs} ${appConfig.UI_LABELS.hr}`
              : `${hrs} ${appConfig.UI_LABELS.hr} ${rem} ${appConfig.UI_LABELS.min}`;
        } else {
...
export function updateTurnUI(currentFloatIdx) {
  ...
  const dist = distanceToIndexFraction(
    currentFloatIdx,
    next.endIdx,
    routeCoords
  );
  ...
  const formattedDist =
    typeof appConfig.TOP_TURN_KM_THRESHOLD === "number" &&
    dist >= appConfig.TOP_TURN_KM_THRESHOLD
  ...
  try {
    ...
    if (placeText && appConfig.TOP_TURN_SHOW_PLACE) {
      ...
      if (
        containsCJK(placeText) &&
        next.step.maneuver &&
        next.step.maneuver.location
      ) {
        romanizePlaceNameIfNeeded(placeText, next.step.maneuver.location).then(
          (r) => {
            const short =
              r.length > appConfig.TOP_TURN_PLACE_MAX_CHARS
                ? r.slice(0, appConfig.TOP_TURN_PLACE_MAX_CHARS - 1) + "…"
                : r;
            try {
              topTurnPlace.textContent = `At: ${short}`;
              topTurnPlace.style.display = "";
            } catch (e) {
              console.warn("Failed to update topTurnPlace", e);
            }
          }
        );
      }
      const short =
        display.length > appConfig.TOP_TURN_PLACE_MAX_CHARS
          ? display.slice(0, appConfig.TOP_TURN_PLACE_MAX_CHARS - 1) + "…"
          : display;
      topTurnPlace.textContent = `At: ${short}`;
      topTurnPlace.style.display = "";
    } else {
    ...
    const svg = getManeuverIconSVG(
      m.modifier,
      m.type,
      appConfig.TOP_TURN_ICON_SIZE_PX
    );
    ...
    /* non-critical UI failure — ignore */
  }
}
