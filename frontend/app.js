...
import {
  ...
  registerMarker,
  removeMarker,
  setDriversLayerGroup,
  getDriversLayerGroup,
...
Object.assign(window.__tweakables, {
  ...
  setRouteColor(c) {
    ...
    if (window._routeLine)
      window._routeLine.setStyle({ color: appConfig.GUI_ROUTE_COLOR });
  },
  setRouteWeight(w) {
    appConfig.GUI_ROUTE_WEIGHT_PX = w;
    if (window._routeLine)
      window._routeLine.setStyle({ weight: appConfig.GUI_ROUTE_WEIGHT_PX });
  },
  ...
  setSpeedMultiplier(m) {
    appConfig.GUI_SPEED_REALISM_MULTIPLIER = Number(m);
  },
  setMaxAccel(a) {
    appConfig.MAX_ACCEL = Number(a);
  },
  setMaxDecel(d) {
    appConfig.MAX_DECEL = Number(d);
  },
  setTurboStrictness(s) {
    appConfig.TURBO_STRICTNESS = Number(s);
  },
  setHighwayMinKmph(n) {
    appConfig.GUI_HIGHWAY_MIN_KMPH = Number(n);
  },
  setShowCountry(b) {
    appConfig.GUI_SHOW_COUNTRY = Boolean(b);
    updateHUD();
  },
  setShowInfoPanel(b) {
    appConfig.GUI_INFO_PANEL_VISIBLE = Boolean(b);
    import("./ui.js")
      .then((m) => m.setInfoPanelVisible(appConfig.GUI_INFO_PANEL_VISIBLE))
      .catch(() => {});
  },
  setCountryGeocodeDebounceMs(ms) {
    appConfig.COUNTRY_REVERSE_GEOCODE_DEBOUNCE_MS = Number(ms);
  },
  setCountryGeocodeMinDistM(m) {
    appConfig.COUNTRY_REVERSE_GEOCODE_MIN_DIST_M = Number(m);
  },
});
...
(async function initApp() {
  try {
    ...
    const res = await fetch(
      `${appConfig.OSM_NOMINATIM_BASE}/reverse?format=jsonv2&lat=${appConfig.TOKYO.lat}&lon=${appConfig.TOKYO.lng}`
    );
    if (res.ok) {
      const j = await res.json();
      const cc =
        j.address && j.address.country_code
          ? j.address.country_code.toUpperCase()
          : null;
...
map.on("click", async (e) => {
  try {
    ...
    addMarker(
      dest.lat,
      dest.lng,
      `Clicked: ${dest.lat.toFixed(5)}, ${dest.lng.toFixed(5)}`
    );
    ...
    if (appConfig.GUI_TURBO_MODE) {
      try {
        route = await fetchRouteAlternatives(
          { lat: fromLatLng.lat, lng: fromLatLng.lng },
          dest,
          appConfig.TURBO_MAX_ALTERNATIVES
        );
      } catch (altErr) {
        console.warn(
          "Turbo alternatives failed, falling back to single route",
          altErr
        );
    ...
    try {
      ...
      const anyStepIsHighway = (route.legs || []).some((leg) =>
        (leg.steps || []).some((s) => {
    ...
    try {
      ...
      console.warn("Failed to set route data attribute", e);
    }
...
function updateConnectionHistory() {
  ...
  connectionHistory.forEach((conn) => {
    ...
    entry.textContent = `${conn.date.toLocaleDateString()} - ${conn.date.toLocaleTimeString()} (${durationMin}m ${pad(
      durationSec
    )}s)`;
  ...
  try {
    ...
    json.drivers.forEach((d) => {
      ...
      if (!existing) {
        ...
        const m = L.marker([lat, lon], { icon }).bindTooltip(id, {
          direction: "top",
          offset: [0, -10],
        });
...
function startDriversPolling() {
  ...
  const ms =
    settings && settings.pollInterval
      ? parseInt(settings.pollInterval, 10)
      : 2000;
...
function loadRuntimeSettings() {
  ...
  pollToggleBtn.textContent = driversPollingInterval
    ? "Stop polling"
    : "Start polling";

  if (driversPollingInterval) {
    clearInterval(driversPollingInterval);
    driversPollingInterval = setInterval(
      fetchAndRenderDrivers,
      parseInt(pollIntervalInput.value, 10)
    );
  }
}
...
function toggleDriversPolling() {
  ...
  persistRuntimeSettingsToStorage({
    ...
    simBatchSize: parseInt(
      document.getElementById("sim-batch-size").value || "50",
      10
    ),
    rustAddrs: (
      document.getElementById("rust-addrs").value || "http://127.0.0.1:3030"
    ).trim(),
  });
}
...
function openRustMetrics() {
  const settings = JSON.parse(localStorage.getItem("runtimeSettings") || "{}");
  const addrs = (settings.rustAddrs || "http://127.0.0.1:3030")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
...
function startSimulatorFromUI() {
  ...
  const count = parseInt(
    document.getElementById("sim-count").value || "100",
    10
  );
  const settings = JSON.parse(localStorage.getItem("runtimeSettings") || "{}");
  const rustAddrs = settings.rustAddrs || "http://127.0.0.1:3030";
  fetch("/generate-drivers?count=" + encodeURIComponent(Math.max(1, count)))
    .then((r) => {
      if (r.ok) {
        document.getElementById(
          "gear-output"
        ).textContent = `Generator started: count=${count}`;
      } else {
        document.getElementById(
          "gear-output"
        ).textContent = `Generator unavailable (HTTP ${r.status})`;
      }
    })
    .catch((e) => {
      document.getElementById("gear-output").textContent =
        "Error contacting generator: " + e.message;
    });
}

function stopSimulatorFromUI() {
  document.getElementById("gear-output").textContent =
    "Stop simulator request sent (if available).";
}
...
async function returnTrackedData() {
  ...
  const selection = {
    location:
      formData.get("trackLocation") === "on" || formData.has("trackLocation"),
    speed: formData.get("trackSpeed") === "on" || formData.has("trackSpeed"),
    heading:
      formData.get("trackHeading") === "on" || formData.has("trackHeading"),
  ...
  try {
    ...
    if (selection.speed || selection.heading) {
      ...
      if (pos) {
        if (selection.speed)
          data.speed_m_s = pos.coords.speed === null ? null : pos.coords.speed;
        if (selection.heading)
          data.heading_deg =
            pos.coords.heading === null ? null : pos.coords.heading;
      }
    }
    ...
    fileInput.addEventListener("change", async (e) => {
      ...
      if (!uploadGpxFile) {
        resultDiv.textContent =
          "Upload unavailable (client module failed to load)";
        return;
      }
      try {
